...
*/
	...
	public List findAll()
	{
		...
		try
		{
			String queryString = "from TGonggao order by gonggaoId desc";// order by gonggaoId desc
	...
	//分页查询  
	  public List<TGonggao> queryByPage (int pageSize, int pageNow) {  
	    //List<TGonggao> list = new ArrayList<TGonggao> () ;  
	    try {  	   
	    	 String queryString =
	 	            "select * from TGonggao order by gonggaoId desc limit "+(pageNow*pageSize-pageSize)+","+pageSize;         	  
	 	     return getHibernateTemplate().find(queryString);	     		
	    } catch (RuntimeException re)
		{
			log.error("find all failed", re);
